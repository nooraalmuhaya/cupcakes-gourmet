...
import { esc, moeda, dataHora, formatarTelefone, seloStatus, htmlCarregando, mostrarErroCarregamento, comCarregamento, confirmar, toast, param, icone, estrelasLeitura } from "../../ui.js";
...
const ICONE_ACAO = { EM_PREPARO: "preparo", SAIU_PARA_ENTREGA: "entrega", ENTREGUE: "entregue" };
...
function caixaStatus(p) {
  ...
  const passos = p.linha_do_tempo.map((e, i) => `<li class="${i <= posicao ? "feito" : ""} ${e.atual ? "atual" : ""}">
      <span class="bolinha" aria-hidden="true">${i <= posicao ? icone("check") : ""}</span>${esc(e.texto)}${e.atual ? ' <span class="texto-pequeno" style="margin-left:auto;color:var(--color-primary-strong)">atual</span>' : ""}</li>`).join("");
  let acoes = "";
  if (p.proximo_status) {
    acoes += `<button type="button" class="botao botao-bloco" data-proximo="${p.proximo_status}">${icone(ICONE_ACAO[p.proximo_status])}${esc(ROTULO_ACAO[p.proximo_status])}</button>`;
  }
    ...
    <div class="grade-admin">
        ...
        <div class="coluna-dupla" style="align-items:start">
          <div><h2>Pagamento</h2><p style="margin:0 0 6px">${NOME_METODO[p.pagamento.metodo]}</p>
            <span class="status status-${p.pagamento.status === "APROVADO" ? "ENTREGUE" : p.pagamento.status === "RECUSADO" ? "CANCELADO" : "AGUARDANDO_PAGAMENTO"}">
              ${NOME_STATUS_PAGAMENTO[p.pagamento.status]}</span></div>
      ...
      <div class="pilha">
        ...
        <section class="cartao"><h2>Avaliação do cliente</h2>
          ${aval ? `<p style="margin:0">${estrelasLeitura(aval.nota)}
